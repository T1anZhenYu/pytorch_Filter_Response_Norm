...
class DetachVarFunction(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x, running_mean, running_var, eps, momentum):
        n = x.numel() / (x.size(1))
        # running_mean = running_mean.double()
        ...
        mean = mean.squeeze()
        var = var.squeeze()
        var = torch.clamp(var, min=0.05, max=1000000)
        running_mean.copy_(momentum * mean \
                           + (1 - momentum) * running_mean)
        # update running_var with unbiased var
        running_var.copy_(momentum * var * n / (n - 1) \
...
class DetachVarNorm(nn.BatchNorm2d):
    def __init__(self, num_features, eps=0.00001, momentum=0.1,
                 affine=True, track_running_stats=True):
        super(DetachVarNorm, self).__init__(
            num_features, eps, momentum, affine, track_running_stats)
    ...
    def forward(self, x):
        ...
        x = x.double()
        if self.training:
            y = DetachVarFunction.apply(x, self.running_mean, self.running_var, self.eps, self.momentum)
        else:
            mean = self.running_mean
            var = self.running_var
            y = (x - mean[None, :, None, None]) / (torch.sqrt(var[None, :, None, None] + self.eps))
...
class DetachClipRangeBN(nn.Module):
    def __init__(self, num_features, eps=1e-05, momentum=0.9, affine=True):
        """
        Input Variables:
        ----------------
        ...
        super(DetachClipRangeBN, self).__init__()
        self.affine = affine
        if self.affine:
            self.bias = nn.parameter.Parameter(
                torch.Tensor(1, num_features, 1, 1), requires_grad=True)
...
class DetachClipRangeBNNoMean(nn.Module):
    def __init__(self, num_features, eps=1e-05, momentum=0.9, affine=True):
        """
        Input Variables:
        ----------------
        ...
        super(DetachClipRangeBNNoMean, self).__init__()
        self.affine = affine
        if self.affine:
            self.bias = nn.parameter.Parameter(
                torch.Tensor(1, num_features, 1, 1), requires_grad=True)
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            self.running_var.copy_(self.momentum * var \
                                   + (1 - self.momentum) * self.running_var)
            y = (x) / (torch.sqrt(var[None, :, None, None] + self.eps))
...
class VarLearn(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            var_ = ((x - mean).pow(2).mean(dim=(0, 2, 3), keepdim=True)).detach()
            print(mean(var_))
            var = self.uplimit
            # print(var.shape)
            self.running_mean.copy_(self.momentum * mean \
